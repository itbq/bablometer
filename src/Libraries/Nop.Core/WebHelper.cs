...
using Nop.Core.IISConfigurerServiceClient;
...
namespace Nop.Core
{
    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string AddWebSiteBinding(string domainName, string storeUrl)
        {
            IIISConfigurer client = new IISConfigurerClient();
            client.AddBinding(domainName);
            if (domainName.IndexOf(storeUrl) > 0)
            {
                domainName = domainName.Substring(0, domainName.IndexOf("."));
                return client.AddSubdominNSRecord(domainName);
            }
            return "";
        }

        public virtual string DeleteWebSiteBinding(string domainName, string storeUrl)
        {
            IIISConfigurer client = new IISConfigurerClient();
            client.DeleteBinding(domainName);
            if (domainName.IndexOf(storeUrl) > 0)
            {
                domainName = domainName.Substring(0, domainName.IndexOf("."));
                return client.DeleteSubdomain(domainName);
            }
            return "";
        }

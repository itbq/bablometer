...
namespace Nop.Services.Seo
{
    ...
    public partial class UrlRecordService : IUrlRecordService
    {
        ...
        public virtual void ClearEntitySlug<T>(T entity, int languageId)where T : BaseEntity, ISlugSupported
        {
            if (entity == null)
            {
                throw new ArgumentNullException("entity");
            ...
            string entityName = typeof(T).Name;
            var slugs = _urlRecordRepository.Table.Where(x => x.EntityId == entity.Id && x.EntityName == entityName).ToList();
            if (languageId != 0)
            {
                var slug = slugs.Where(x => x.LanguageId == languageId).FirstOrDefault();
                if (slug != null)
                {
                    _urlRecordRepository.Delete(slug);
                }
            }
            else
            {

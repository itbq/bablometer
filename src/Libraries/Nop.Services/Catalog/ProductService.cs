...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// <summary>
        /// Get recently added products to catalog.
        /// </summary>
        /// <returns></returns>
        public virtual IList<Product> GetRecentlyAddedProducts(int count = 15)
        {
            var query = _productRepository.Table.Where(i => !i.Deleted).OrderByDescending(i => i.Id).Take(count);
            var products = query.ToList();
            return products;
        }
        ...
        public virtual IPagedList<Product> NewSearchProducts(int customerId,
            ...
            int languageId)
        {
            ...
            pCategoryId.ParameterName = "CategoryIds";
            ...
            var pProductItemTypeId = _dataProvider.GetParameter();
            pProductItemTypeId.ParameterName = "ProductItemTypeId";
            pProductItemTypeId.Value = productItemTypeId;
            pProductItemTypeId.DbType = DbType.Int32;
        ...
        public virtual IPagedList<Product> SearchProductsWithAttributes(int customerId,
            ProductSortingEnum orderBy,
            int pageIndex,
            int pageSize,
            int productTagId,
            ...
            int productRating,
            int regionId,
            IList<SearchProductAttributeValue> productAttributeValues,
            IList<SearchProductAttributeValue> customerAttributeValues)
        {
            var pCustomerId = _dataProvider.GetParameter();
            pCustomerId.ParameterName = "CustomerId";
            pCustomerId.Value = customerId;
            ...
            var pBankRating = _dataProvider.GetParameter();
            pBankRating.ParameterName = "BankRating";
            pBankRating.Value = bankRating;
            pBankRating.DbType = DbType.Int32;

            var pProductRating = _dataProvider.GetParameter();
            pProductRating.ParameterName = "ProductRating";
            pProductRating.Value = productRating;
            pProductRating.DbType = DbType.Int32;

            var pRegionId = _dataProvider.GetParameter();
            pRegionId.ParameterName = "RegionId";
            pRegionId.Value = regionId;
            pRegionId.DbType = DbType.Int32;
            ...
            var pIntAttributes = (SqlParameter)_dataProvider.GetParameter();
            pIntAttributes.ParameterName = "IntAttributes";
            pIntAttributes.SqlDbType = SqlDbType.Structured;

            var pRangeAttributes = (SqlParameter)_dataProvider.GetParameter();
            pRangeAttributes.ParameterName = "RangeAttributes";
            pRangeAttributes.SqlDbType = SqlDbType.Structured;

            var pTextAttributes = (SqlParameter)_dataProvider.GetParameter();
            pTextAttributes.ParameterName = "TextAttributes";
            pTextAttributes.SqlDbType = SqlDbType.Structured;

            var pReferenceAttributes = (SqlParameter)_dataProvider.GetParameter();
            pReferenceAttributes.ParameterName = "Attributes";
            pReferenceAttributes.SqlDbType = SqlDbType.Structured;

            if (productAttributeValues != null && productAttributeValues.Count > 0)
            {
                //Real attributes processing
                var intAttributesValues = productAttributeValues.Where(x =>
                    x.ControlType == SearchAttributeControlType.TextBoxReal
                    || x.ControlType == SearchAttributeControlType.ToddlerIntBetween
                    || x.ControlType == SearchAttributeControlType.ToddlerMax
                    || x.ControlType == SearchAttributeControlType.ToddlerMin).ToList();
                
                if (intAttributesValues != null && intAttributesValues.Count > 0)
                {
                    var intAttributesTable = new DataTable();
                    intAttributesTable.Columns.Add("MaxValue", typeof(double));
                    intAttributesTable.Columns.Add("MinValue", typeof(double));
                    intAttributesTable.Columns.Add("ExactValue", typeof(double));
                    intAttributesTable.Columns.Add("CategoryProductAttributeId", typeof(int));
                    intAttributesValues.Where(x => x.ControlType == SearchAttributeControlType.TextBoxReal)
                        .ToList().ForEach(x =>
                            {
                                intAttributesTable.Rows.Add(DBNull.Value, DBNull.Value, x.ExactValue, x.CategoryProductAttributeId);
                            });
                    intAttributesValues.Where(x => x.ControlType == SearchAttributeControlType.ToddlerIntBetween)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(x.MaxValue, x.MinValue, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    intAttributesValues.Where(x => x.ControlType == SearchAttributeControlType.ToddlerMax)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(x.MaxValue, DBNull.Value, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    intAttributesValues.Where(x => x.ControlType == SearchAttributeControlType.ToddlerMin)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(DBNull.Value, x.MinValue, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    pIntAttributes.SqlValue = intAttributesTable;
                }

                //Money range attributes
                var rangeAttributes = productAttributeValues.Where(x => x.ControlType == SearchAttributeControlType.Money);
                if (rangeAttributes != null && rangeAttributes.Count() > 0)
                {
                    var intAttributesTable = new DataTable();
                    intAttributesTable.Columns.Add("MaxValue", typeof(double));
                    intAttributesTable.Columns.Add("MinValue", typeof(double));
                    intAttributesTable.Columns.Add("ExactValue", typeof(double));
                    intAttributesTable.Columns.Add("CategoryProductAttributeId", typeof(int));
                    rangeAttributes.ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(DBNull.Value, DBNull.Value, x.MinValue, x.CategoryProductAttributeId);
                        });
                    pRangeAttributes.SqlValue = intAttributesTable;
                }
                //Text attributes processing
                var textBoxAttributes = productAttributeValues.Where(x => x.ControlType == SearchAttributeControlType.TextBoxText);
                if (textBoxAttributes != null && textBoxAttributes.Count() > 0)
                {
                    var textAttributesTable = new DataTable();
                    textAttributesTable.Columns.Add("AttributeControlTypeId", typeof(int));
                    textAttributesTable.Columns.Add("AttributeValue", typeof(string));
                    textBoxAttributes.ToList().ForEach(x =>
                        {
                            textAttributesTable.Rows.Add(x.CategoryProductAttributeId, x.Textvalue);
                        });
                    pTextAttributes.SqlValue = textAttributesTable;
                }

                //Reference attribuutes processing
                var referenceAttributes = productAttributeValues.Where(x => x.ControlType == SearchAttributeControlType.CheckBox ||
                    x.ControlType == SearchAttributeControlType.CheckBoxGroup ||
                    x.ControlType == SearchAttributeControlType.DropDown ||
                    x.ControlType == SearchAttributeControlType.CheckBox);
               
                if (referenceAttributes != null && referenceAttributes.Count() > 0)
                {
                    var referenceAttributesTable = new DataTable();
                    referenceAttributesTable.Columns.Add("AttributeId", typeof(int));
                    referenceAttributes.ToList().ForEach(x =>
                    {
                        referenceAttributesTable.Rows.Add(x.IdValue);
                    });
                    pReferenceAttributes.SqlValue = referenceAttributesTable;
                }
            }

            //Customer attributes processing
            var pCustomerIntAttributes = (SqlParameter)_dataProvider.GetParameter();
            pCustomerIntAttributes.ParameterName = "CustomerIntAttributes";
            pCustomerIntAttributes.SqlDbType = SqlDbType.Structured;

            var pCustomerTextAttributes = (SqlParameter)_dataProvider.GetParameter();
            pCustomerTextAttributes.ParameterName = "CustomerTextAttributes";
            pCustomerTextAttributes.SqlDbType = SqlDbType.Structured;

            var pCustomerReferenceAttributes = (SqlParameter)_dataProvider.GetParameter();
            pCustomerReferenceAttributes.ParameterName = "CustomerAttributes";
            pCustomerReferenceAttributes.SqlDbType = SqlDbType.Structured;


            if (customerAttributeValues != null && customerAttributeValues.Count > 0)
            {
                //Real attributes processing
                var intAttributesValues = customerAttributeValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.NumberTextBoxExact
                    || x.CustomerControlType == CustomerInformationProductSearchControlType.NumberToddlerBetween
                    || x.CustomerControlType == CustomerInformationProductSearchControlType.NumberToddlerLess
                    || x.CustomerControlType == CustomerInformationProductSearchControlType.NumberToddlerMore
                    || x.CustomerControlType == CustomerInformationProductSearchControlType.MoneyLess ||
                        x.CustomerControlType == CustomerInformationProductSearchControlType.MoneyMore ||
                        x.CustomerControlType == CustomerInformationProductSearchControlType.MonyBetween).ToList();
                ...
                if (intAttributesValues != null && intAttributesValues.Count > 0)
                {
                    ...
                    intAttributesTable.Columns.Add("ExactValue", typeof(double));
                    intAttributesTable.Columns.Add("CategoryProductAttributeId", typeof(int));
                    intAttributesValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.NumberTextBoxExact)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(DBNull.Value, DBNull.Value, x.ExactValue, x.CategoryProductAttributeId);
                        });
                    intAttributesValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.NumberToddlerBetween)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(x.MaxValue, x.MinValue, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    intAttributesValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.NumberToddlerLess)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(x.MaxValue, x.MaxValue, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    intAttributesValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.NumberToddlerMore)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(x.MinValue, x.MinValue, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    intAttributesValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.MoneyLess ||
                        x.CustomerControlType == CustomerInformationProductSearchControlType.MoneyMore ||
                        x.CustomerControlType == CustomerInformationProductSearchControlType.MonyBetween)
                        .ToList().ForEach(x =>
                        {
                            intAttributesTable.Rows.Add(DBNull.Value, x.MinValue, DBNull.Value, x.CategoryProductAttributeId);
                        });
                    pCustomerIntAttributes.SqlValue = intAttributesTable;
                }

                //Text attributes processing
                var textBoxAttributes = customerAttributeValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.TextBox);
                if (textBoxAttributes != null && textBoxAttributes.Count() > 0)
                {
                    var textAttributesTable = new DataTable();
                    textAttributesTable.Columns.Add("AttributeControlTypeId", typeof(int));
                    ...
                    pCustomerTextAttributes.SqlValue = textAttributesTable;
                }

                //Reference attribuutes processing
                var referenceAttributes = customerAttributeValues.Where(x => x.CustomerControlType == CustomerInformationProductSearchControlType.Checkboxes ||
                    x.CustomerControlType == CustomerInformationProductSearchControlType.DropDownList ||
                    x.CustomerControlType == CustomerInformationProductSearchControlType.Gender ||
                    x.CustomerControlType == CustomerInformationProductSearchControlType.ToddlerString);
                ...
                if (referenceAttributes != null && referenceAttributes.Count() > 0)
                {
                    ...
                    pCustomerReferenceAttributes.SqlValue = referenceAttributesTable;
                }
            }
            ...
            var products = _dbContext.ExecuteStoredProcedureList<Product>(
                    ...
                    pRangeAttributes,
                    pCustomerTextAttributes,
                    pCustomerIntAttributes,
                    pCustomerReferenceAttributes);
            //return products
            int totalRecords = (pTotalRecords.Value != DBNull.Value) ? Convert.ToInt32(pTotalRecords.Value) : 0;
            return new PagedList<Product>(products, pageIndex, pageSize, totalRecords);
        }
        ...
        public void DeleteTmpProduct(DateTime endDate)
        {
            var products = _productRepository.Table.Where(x => x.Name == "tmp_product" && x.CreatedOnUtc < endDate);

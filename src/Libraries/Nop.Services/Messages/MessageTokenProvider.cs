...
namespace Nop.Services.Messages
{
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        {
            string languagecode =_languageService.GetLanguageById(languageId).UniqueSeoCode + "/";
            ...
            //TODO add a method for getting URL (use routing because it handles all SEO friendly URLs)
            var productUrl = string.Format("{0}{1}", _webHelper.GetStoreLocation(false) + languagecode, product.GetSeName(languageId));
        ...
        public virtual void AddRequestReqsponceTokens(IList<Token> tokens, Request request, int languageId)
        {
            tokens.Add(new Token("Request.ProposeComment", request.ProposeComment));
            tokens.Add(new Token("Request.ResponceComment", request.ResponseComment));
            tokens.Add(new Token("Request.Accepted",request.Accepted.Value?_localizationService.GetResource("Request.Email.Accepted"):_localizationService.GetResource("Request.Email.Rejected")));
            string companyName = request.Product.Customer.CompanyInformation.GetLocalized(x=>x.CompanyName,languageId,false,false);
            if(companyName == null)
            {
                var languages = new OrderedLanguageCultures();
                var aviableLanguages = _languageService.GetAllLanguages();
                for (int i = 0; i < languages.Cultures.Count; i++)
                {
                    var lang = aviableLanguages.Where(x => x.LanguageCulture == languages.Cultures[i]).FirstOrDefault().Id;
                    companyName = request.Product.Customer.CompanyInformation.GetLocalized(x => x.CompanyName, lang, false, false);
                    if(companyName != null)
                        break;
                }
            }
            tokens.Add(new Token("Request.CompanyName", companyName));
        }

        public virtual void AddNewRequestTokens(IList<Token> tokens, Request request,int languageId)
        {
            tokens.Add(new Token("Request.ProposeComment", request.ProposeComment));
            string companyName = request.Customer.CompanyInformation.GetLocalized(x => x.CompanyName, languageId, false, false);
            if (companyName == null)
            {
                var languages = new OrderedLanguageCultures();
                var aviableLanguages = _languageService.GetAllLanguages();
                ...
                {
                    var lang = aviableLanguages.Where(x => x.LanguageCulture == languages.Cultures[i]).FirstOrDefault().Id;
                    companyName = request.Customer.CompanyInformation.GetLocalized(x => x.CompanyName, lang, false, false);
                    if (companyName != null)
                        break;
                }
            }
        ...
        public virtual void AddNewPublishingToken(IList<Token> tokens, NewsItem newItem)
        {
            tokens.Add(new Token("New.Title",newItem.Title));
            tokens.Add(new Token("Publish.Date",newItem.PublishingDate.Value.ToString("d")));
        }

        public virtual void AddNewMiniSiteTokens(IList<Token> tokens, UserMiniSite miniSite)
        {
            tokens.Add(new Token("MiniSite.Domain", miniSite.DomainName));
            tokens.Add(new Token("MiniSite.UserName",miniSite.Customer.Username));
        }
        ...
        public virtual string[] GetListOfCampaignAllowedTokens()
        {
            var allowedTokens = new List<string>()
            {
                ...
                "%Store.RecentProductBuyingRequests%"
            };
        ...
        public virtual string[] GetListOfAllowedTokens()
        {
            var allowedTokens = new List<string>()
            {
                ...
                "%Product.PictureUrl%",
                "%Request.CompanyName%",
                "%Request.ProposeComment%",
                "%Request.Accepted%",
                "%Request.ResponceComment%",
                "%New.Title%",
                "%Publish.Date%",
                "%Catalog.Upload.Items%",
                "%Catalog.Upload.Category%",
                "%MiniSite.Domain%",
                "%MiniSite.UserName%",
                "%MiniSite.Company%",
            };
        ...
        public virtual string[] GetListOfRecentProductsTokens()
        {
        ...
        public virtual void AddCustomProductTokens(IList<Token> tokens, Product product, int languageId)
        {
            string languagecode =_languageService.GetLanguageById(languageId).UniqueSeoCode + "/";
            ...
            foreach (var category in categories)
            {
                ...
                {
                    sb.Insert(0, "-<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + cat.GetSeName(languageId) + "\">" + cat.GetLocalized(c => c.Name, languageId) + "</a>", 1);
            ...
            var picture = product.ProductPictures.Where(x=>x.DisplayOrder == 0).FirstOrDefault();
            if (picture == null)
            {
                picture = product.ProductPictures.FirstOrDefault();
            }
            string pictureurl = picture == null ? null:_pictureService.GetPictureUrl(picture.PictureId,100);
            tokens.Add(new Token("Product.PictureUrl", pictureurl == null ? "" : "<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + product.GetSeName(languageId) + "\"><img src=\"" + pictureurl + "\" title=\"" + product.GetLocalized(pr => pr.Name, languageId) + "\" /></a>"));
            tokens.Add(new Token("Product.Categories",sb.ToString()));
            string companyName = "";
            if (product.Customer.CompanyInformation.GetLocalized(d => d.CompanyName, languageId) != null)
            {
                tokens.Add(new Token("Product.Company", "<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + product.Customer.CompanyInformation.GetSeName(languageId) + "\">" + product.Customer.CompanyInformation.GetLocalized(d => d.CompanyName, languageId) + "</a>"));
            }
            else
            {
                companyName = product.Customer.CompanyInformation.GetLocalized(d => d.CompanyName, _languageService.GetAllLanguages().Where(x => x.LanguageCulture == "es-MX").FirstOrDefault().Id, false);
                if (companyName != null)
                {
                    tokens.Add(new Token("Product.Company", "<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + product.Customer.CompanyInformation.GetSeName(languageId) + "\">" + companyName + "</a>"));
                }
                companyName = product.Customer.CompanyInformation.GetLocalized(d => d.CompanyName, _languageService.GetAllLanguages().Where(x => x.LanguageCulture == "de-DE").FirstOrDefault().Id, false);
                if (companyName != null)
                {
                    tokens.Add(new Token("Product.Company", "<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + product.Customer.CompanyInformation.GetSeName(languageId) + "\">" + companyName + "</a>"));
                }
                companyName = product.Customer.CompanyInformation.GetLocalized(d => d.CompanyName, _languageService.GetAllLanguages().Where(x => x.LanguageCulture == "en-US").FirstOrDefault().Id, false);
                if (companyName != null)
                {
                    tokens.Add(new Token("Product.Company", "<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + product.Customer.CompanyInformation.GetSeName(languageId) + "\">" + companyName + "</a>"));
                }
                companyName = product.Customer.CompanyInformation.GetLocalized(d => d.CompanyName, _languageService.GetAllLanguages().Where(x => x.LanguageCulture == "ru-RU").FirstOrDefault().Id, false);
                if (companyName != null)
                {
                    tokens.Add(new Token("Product.Company", "<a href=\"" + _webHelper.GetStoreLocation(false) + languagecode + product.Customer.CompanyInformation.GetSeName(languageId) + "\">" + companyName + "</a>"));
                }

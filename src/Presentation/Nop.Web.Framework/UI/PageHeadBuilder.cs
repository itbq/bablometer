...
namespace Nop.Web.Framework.UI
{
    public partial class PageHeadBuilder : IPageHeadBuilder
    {
        ...
        private readonly List<string> _metaOgImageParts;
        ...
        public PageHeadBuilder(SeoSettings seoSettings, HttpContextBase httpContext,IWorkContext workContext)
        {
            this._seoSettings = seoSettings;
            this._titleParts = new List<string>();
            this._metaDescriptionParts = new List<string>();
            this._metaKeywordParts = new List<string>();
            this._metaOgImageParts = new List<string>();
        ...
        public virtual string GenerateTitle(bool addDefaultTitle)
        {
            ...
            if (!String.IsNullOrEmpty(specificTitle))
            {
                if (addDefaultTitle)
                {
                    ...
                    else
                    {
                        ...
                        switch (_seoSettings.PageTitleSeoAdjustment)
                        {
                            case PageTitleSeoAdjustment.PagenameAfterStorename:
                                {
                                    result = string.Join(_seoSettings.PageTitleSeparator, _workContext.CurrentMiniSite.MiniSiteLayout.GetLocalized(x=>x.RootTitle,_workContext.WorkingLanguage.Id), specificTitle);
                                }
                                break;
                            case PageTitleSeoAdjustment.StorenameAfterPagename:
                            default:
                                {
                                    result = string.Join(_seoSettings.PageTitleSeparator, specificTitle, _workContext.CurrentMiniSite.MiniSiteLayout.GetLocalized(x => x.RootTitle, _workContext.WorkingLanguage.Id));
                                }
                                break;
                ...
                else
                {
                    ...
                    {
                        result = _workContext.CurrentMiniSite.MiniSiteLayout.GetLocalized(x => x.RootTitle, _workContext.WorkingLanguage.Id);
                    }
                }
            }
            else
        ...
        public virtual void AddOgImageFileParts(string part)
        {
            if (string.IsNullOrEmpty(part))
                return;

            _metaOgImageParts.Add(part);
        }
        public virtual void AppendOgImageFileParts(string part)
        {
            if (string.IsNullOrEmpty(part))
                return;

            _metaOgImageParts.Insert(0,part);
        }
        public virtual string GenerateOgImageFiles()
        {
            if (_metaOgImageParts.Count == 0)
            {
                var settings = EngineContext.Current.Resolve<StoreInformationSettings>();
                return String.Format("<meta property=\"og:image\" content=\"{0}\">", settings.StoreUrl + "Content/etf/images/logo.png");
            }
            //use only distinct rows
            var result = new StringBuilder();
            foreach (var path in _metaOgImageParts)
            {
                result.AppendFormat("<meta property=\"og:image\" content=\"{0}\">", path);
                result.Append(Environment.NewLine);
            }
            return result.ToString();
        }

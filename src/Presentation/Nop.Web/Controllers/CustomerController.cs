...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BaseNopController
    {
        ...
        public CustomerController(IRegionService regionService, ICityService cityService,
        ...
        protected void PrepareCustomerInfoModel(CustomerInfoModel model, Customer customer, bool excludeProperties)
        {
            ...
            model.CompanyDescription = customer.GetAttribute<string>(SystemCustomerAttributeNames.CompanyDescription);
            model.CompanyLogo = new PictureModel (){
                    ImageUrl = _pictureService.GetPictureUrl(customer.GetAttribute<int>(SystemCustomerAttributeNames.PictureId),100)
            };
            if (!excludeProperties)
            {
                ...
                //newsletter
                var newsletter = _newsLetterSubscriptionService.GetNewsLetterSubscriptionByEmail(customer.Email,_workContext.WorkingLanguage.Id);
        ...
        public ActionResult CheckLogin(LoginModel model)
        {
            if (_customerRegistrationService.ValidateCustomer(model.Username, model.Password))
            {
                ...
                return Json(new { correct = true, username = model.Username, password = model.Password, lang = customer.LanguageId });
            }
            return Json(new { correct = false, username = model.Username, password = model.Password});
        }
        ...
        public ActionResult Login(LoginModel model, string returnUrl, bool captchaValid)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_customerRegistrationService.ValidateCustomer(model.Username, model.Password))
                {
                    var customer =  _customerService.GetCustomerByEmail(model.Username);
        ...
        [NopHttpsRequirement(SslRequirement.Yes)]
        public ActionResult Register(bool? buyer)
        {
            ...
            if (buyer.HasValue)
            {
                model.AccountType = buyer.Value ? "B" : "S";
            }
            //form fields
            model.DateOfBirthEnabled = true;
            model.AviableCurrencies = _currencyService.GetAllCurrencies().Select(x => new CurrencyModel() { Name = x.CurrencyCode, Id = x.Id }).ToList();
            model.AcceptPrivacyPolicyEnabled = _customerSettings.AcceptPrivacyPolicyEnabled;
            model.UsernamesEnabled = _customerSettings.UsernamesEnabled;
            model.CheckUsernameAvailabilityEnabled = _customerSettings.CheckUsernameAvailabilityEnabled;
            model.DisplayCaptcha = _captchaSettings.Enabled && _captchaSettings.ShowOnRegistrationPage;
            model.Regions = _regionService.GetAllRegions().OrderBy(x=>x.Title).Select(x => new RegionModel()
            {
                Title = x.Title,
                Id = x.Id
            }).ToList();
        ...
        public ActionResult Register(RegisterModel model, string returnUrl, bool captchaValid)
        {
            ...
            {
                ModelState.AddModelError("Password", _localizationService.GetResource("Profile.Password.Space"));
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (registrationResult.Success)
                {
                    ...
                    if (model.DateOfBirthDay.Value != 0 && model.DateOfBirthYear.Value != 0 && model.DateOfBirthMonth.Value != 0)
                    {
                        var birthdayDate = new DateTime(model.DateOfBirthYear.Value, model.DateOfBirthMonth.Value, model.DateOfBirthDay.Value);
                        customer.BirthdayDate = birthdayDate;
                    }
                    ...
                    if (model.Income.HasValue)
                    {
                        ...
                        customer.Income = (double)((double)model.Income / (double)currency.Rate);
                        customer.CurrencyId = model.CurrencyId;
                    }
                    customer.Address = model.Address;
                    _customerService.UpdateCustomer(customer);
                    customer.CustomerRoles.Add(_customerService.GetCustomerRoleBySystemName("Buyers"));
                    //form fields
                ...
                else
                {
                    foreach (var error in registrationResult.Errors)
                    {
                        ...
                        if (error == _localizationService.GetResource("Account.Register.Errors.UsernameAlreadyExists"))
                        {
                            ModelState.AddModelError("Email", error);
                        }
            ...
            model.Cities = new List<CityModel>();
             model.Regions.Add(new RegionModel() { Id = 0, Title = _localizationService.GetResource("ITB.Portal.Register.SelectRegion") });
             ((List<RegionModel>)model.Regions).AddRange(_regionService.GetAllRegions().OrderBy(x=>x.Title).Select(x => new RegionModel()
                {
                    Title = x.Title,
                    Id = x.Id
                }));
            ...
            {
                model.Cities.Add(new CityModel() { Id = 0, Title = _localizationService.GetResource("ITB.Portal.Register.SelectCity") });
                ((List<CityModel>)model.Cities).AddRange(_regionService.GetById(model.RegionId).Cities.OrderBy(x=>x.Title).Select(x => new CityModel()
                {
                    Title = x.Title,
                    Id = x.Id
                }));
        ...
        public ActionResult GetCities(int regionId)
        {
            ...
            if (region == null)
            {
                ...
                model.Add(new CityModel() { Id = 0, Title = _localizationService.GetResource("ITB.Portal.Register.SelectCity") });
                ((List<CityModel>)model).AddRange(_cityService.GetAllCities().OrderBy(x=>x.Title).Select(x=>new CityModel(){Id = x.Id,Title = x.Title}));
                
                string result = RenderPartialViewToString("GetCities", model);
                return new JsonResult()
                {
                    Data = new {Success = true, resultHtml = result }
                };
            }
            else
            {
                var model = new List<CityModel>();
                model.Add(new CityModel() { Id = 0, Title = _localizationService.GetResource("ITB.Portal.Register.SelectCity") });
                ((List<CityModel>)model).AddRange(region.Cities.OrderBy(x => x.Title).Select(x => new CityModel() { Id = x.Id, Title = x.Title }));
                string result = RenderPartialViewToString("GetCities", model);
                return new JsonResult()
                {
                    Data = new {Success = true, resultHtml = result }
        ...
        [HttpPost]
        public string PasswordRecoverySend(PasswordRecoveryModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    if (customer.ExternalAuthenticationRecords.Count > 0)
                    {
                        model.Result = _localizationService.GetResource("Account.PasswordRecovery.ExternalAccountEmail");
                        return model.Result;
                    }

...
namespace Nop.Admin.Controllers
{
    ...
    public partial class CategoryController : BaseNopController
    {
        ...
        protected void PrepareCategoryAttributeGroups(CategoryModel model, Category category)
        {
            ...
            model.AvailableCategoryProductAttributeGroups = new List<SelectListItem> { new SelectListItem { Text = "[None]", Value = "0" } };
        ...
        public ActionResult List()
        {
            ...
            model.Categories = new GridModel<CategoryModel>
            {
                Data = categories.Select(x =>
                {
                    ...
                    //categoryModel.SeName = x.GetSeName(_workContext.WorkingLanguage.Id);
        ...
        public ActionResult AllCategories(string text, int selectedId)
        {
            ...
            foreach (var c in categories)
                selectList.Add(new SelectListItem()
                    {
                        Value = c.Id.ToString(),
                        Text = c.GetCategoryBreadCrumb(_categoryService,_workContext),
        ...
        public ActionResult Create(CategoryModel model, bool continueEditing)
        {
            ...
            if (model.ParentCategoryId > 0)
            {
                ...
                if (parentCategory != null && !parentCategory.Deleted)
                    model.ParentCategories.Add(new DropDownItem { Text = parentCategory.GetCategoryBreadCrumb(_categoryService,_workContext), Value = parentCategory.Id.ToString() });
                else
        ...
        public ActionResult Edit(int id)
        {
            ...
            PrepareCategoryAttributeGroups(model, category);
            //templates
            PrepareTemplatesModel(model);
            //discounts
        ...
        public ActionResult Edit(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //update connection
                SaveSelectedCategoryProductAttributeGroup(category, model);
                //update picture seo file name
                UpdatePictureSeoNames(category);
                //ACL
                SaveCategoryAcl(category, model);
        ...
        public ActionResult Delete(int id)
        {
            ...
            _urlRecordService.ClearEntitySlug(category, 0);
        ...
        [GridAction(EnableCustomBinding = true)]
        public ActionResult CategoryProductAttributeGroupList(GridCommand command, int categoryId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCatalog))
                return AccessDeniedView();            
            var avalableCategoryGroupAttributes = _categoryProductAttributeService.GetCategoryToCategoryProductAttributeGroupByCategoryId(categoryId);
            var model = new GridModel<CategoryProductAttributeGroupModel>
            {
                Data = avalableCategoryGroupAttributes
                .Select(x =>
                {
                    return new CategoryProductAttributeGroupModel()
                    {
                        Id = x.Id,
                        Name = x.CategoryProductAttributeGroup.Name               
                    };
                }),
                Total = avalableCategoryGroupAttributes.Count
            };
        ...
        [GridAction(EnableCustomBinding = true)]
        public ActionResult CategoryProductAttributeGroupDelete(int id, int categoryId, GridCommand command)
        {
            var catAttrGr = _categoryProductAttributeService.GetCategoryToCategoryProductAttributeGroupById(id);
            if (catAttrGr != null)
            {                
                _categoryProductAttributeService.DeleteCategoryToCategoryProductAttributeGroup(catAttrGr);                
            }
            return CategoryProductAttributeGroupList(command, categoryId);
        }

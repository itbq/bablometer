...
namespace Nop.Admin.Controllers
{
    ...
    public partial class CustomerController : BaseNopController
    {
        ...
        public CustomerController(IRegionService regionService,ICityService cityService, ICustomerService customerService,
        ...
        protected IList<RegisteredCustomerReportLineModel> GetReportRegisteredCustomersModel()
        {
            ...
            report.Add(new RegisteredCustomerReportLineModel()
            {
                ...
                Sellers = _customerReportService.GetRegisteredCustomersReport(7,true),
                Buyers = _customerReportService.GetRegisteredCustomersReport(7, false)
            });

            report.Add(new RegisteredCustomerReportLineModel()
            {
                Period = _localizationService.GetResource("Admin.Customers.Reports.RegisteredCustomers.Fields.Period.14days"),
                Sellers = _customerReportService.GetRegisteredCustomersReport(14, true),
                Buyers = _customerReportService.GetRegisteredCustomersReport(14, false)
            });
            report.Add(new RegisteredCustomerReportLineModel()
            {
                Period = _localizationService.GetResource("Admin.Customers.Reports.RegisteredCustomers.Fields.Period.month"),
                Sellers = _customerReportService.GetRegisteredCustomersReport(30, true),
                Buyers = _customerReportService.GetRegisteredCustomersReport(30, false)
            });
            report.Add(new RegisteredCustomerReportLineModel()
            {
                Period = _localizationService.GetResource("Admin.Customers.Reports.RegisteredCustomers.Fields.Period.year"),
                Sellers = _customerReportService.GetRegisteredCustomersReport(365, true),
                Buyers = _customerReportService.GetRegisteredCustomersReport(365, false)
            });
        ...
        [NonAction]
        protected CustomerModel PrepareCustomerModelForList(Customer customer, IWorkContext _workContext)
        {
        ...
        public ActionResult List()
        {
            ...
            var defaultRoleIds = new[] { _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.Buyers).Id, _customerService.GetCustomerRoleBySystemName(SystemCustomerRoleNames.Administrators).Id };
            ...
            {
                Data = customers.Select(x=>PrepareCustomerModelForList(x,_workContext)),
            ...
            [ModelBinderAttribute(typeof(CommaSeparatedModelBinder))] int[] searchCustomerRoleIds)
        {
            ...
            var customers = _customerService.GetAllCustomers(null, null,
                defaultRoleIds, model.SearchEmail, null,
                null, null,
                0, 0,
                null, null, null,
        ...
        public ActionResult Edit(int id)
        {
            ...
            model.AviableCurrencies = _currencyService.GetAllCurrencies().Select(x =>
                new CurrencyModel()
                {
                    Id=x.Id,
            ...
            model.AvailableCustomerRoles = _customerService
                .GetAllCustomerRoles(true)
                .Where(x => x.SystemName == "Buyers" || x.SystemName == "Administrators" || x.SystemName == "Registered")
                .Select(cr => cr.ToModel())
                .ToList();
            model.SelectedCustomerRoleIds = customer.CustomerRoles.Select(cr => cr.Id).ToArray();
            model.CustomerRoleNames = customer.CustomerRoles.Where(x => x.SystemName == "Sellers").Select(x=>x.SystemName).FirstOrDefault();
            model.AllowManagingCustomerRoles = _permissionService.Authorize(StandardPermissionProvider.ManageCustomerRoles);
            ...
            model.Regions = _regionService.GetAllRegions().Select(x => new RegionModel()
            {
                Title = x.Title,
                Id = x.Id
            }).ToList();
            ...
            model.Cities = _cityService.GetAllCities().Select(x => new CityModel()
            {
                Title = x.Title,
                Id = x.Id
            }).ToList();
        ...
        public ActionResult Edit(CustomerModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    customer.BirthdayDate = model.BirthdayDate;
                    if (model.Income.HasValue)
                    {
                        var currency = _currencyService.GetCurrencyById(model.CurrencyId);
                        customer.Income = (double)(model.Income/currency.Rate);
            ...
            if (model.RegionId != 0)
            {
                model.Cities = _regionService.GetById(model.RegionId).Cities.Select(x => new CityModel()
                {
                    Title = x.Title,
                    Id = x.Id
                }).ToList();
        ...
        public ActionResult Delete(int id)
        {
            ...
            try
            {
                _urlRecordService.ClearEntitySlug(customer.CompanyInformation, 0);
                ...
                if (customer.News != null && customer.News.Count > 0)
                {
                    foreach (var newItem in customer.News.ToList())
                    {
                        _newsService.DeleteNews(newItem);
                    }
                }
        ...
        public ActionResult AddressesSelect(int customerId, GridCommand command)
        {
            ...
            var gridModel = new GridModel<AddressModel>
            {
                ...
                {
                    var model = x.ToModel();
                    model.City = x.GetLocalized(a => a.City, _workContext.WorkingLanguage.Id,false,false);
                    model.Address1 = x.GetLocalized(a => a.Address1, _workContext.WorkingLanguage.Id, false, false);
                    model.FirstName = x.GetLocalized(a => a.FirstName, _workContext.WorkingLanguage.Id, false, false);
        ...
        public ActionResult AddressEdit(int addressId, int customerId)
        {
            ...
            model.Locales = new List<AddressLocalizedModel>();
            AddLocales(_languageService, model.Locales, (locale, languageId) =>
            {
                locale.FirstName = address.GetLocalized(x => x.FirstName, languageId, false, false);
                locale.Address1 = address.GetLocalized(x => x.Address1, languageId, false, false);
                locale.City = address.GetLocalized(x => x.City, languageId, false, false);
            });
        ...
        public ActionResult AddressEdit(AddressModel model)
        {
            ...
            {
                if (ModelState["Email"].Errors.Count == 0 && ModelState["PhoneNumber"].Errors.Count == 0)
                {
                    int countryId = address.CountryId.Value;
                    ...
                    return RedirectToAction("AddressEdit", new { addressId = model.Id, customerId = model.CustomerId });
                }
            }
